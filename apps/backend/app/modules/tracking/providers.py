...
class MockTrackingProvider(TrackingProvider):
    ...
    def get_tracking_events(self, container_no: str, carrier_code: str) -> list[TrackingEventOut]:
        ...
        timeline = [
            ...
            ("Vessel arrival", now + timedelta(days=2), "DEMO Destination A Port", "INPRT", "port"),
            ("Discharged", now + timedelta(days=3), "DEMO Destination A Port", "INPRT", "port"),
            ("Customs hold", now + timedelta(days=3, hours=4), "DEMO Destination A Customs", "INPRT", "port"),
            ("Available for pickup", now + timedelta(days=4), "DEMO Destination A CFS", "INPRT", "port"),
            ("Gate out", now + timedelta(days=5), "DEMO Destination A CFS", "INPRT", "truck"),
            (
                "Rail departure",
                now + timedelta(days=5, hours=6),
                "DEMO Destination A Rail Yard",
                "INPRT",
                "rail",
            ),
            ("Rail arrival", now + timedelta(days=7), "Demo Plant A Rail Terminal", "INIXW", "rail"),
            (
                "Truck appointment",
                now + timedelta(days=7, hours=8),
                "Demo Plant A Plant Gate",
                "INIXW",
                "truck",
            ),
            ("Delivered", now + timedelta(days=8), "Demo Plant A Plant", "INIXW", "truck"),
        ]
        return [
            TrackingEventOut(
